...
class QualityScore(BaseModel):
    ...
    def is_rag_ready(self) -> bool:
        """Return True only when every dimension is ≥ 3.

        Dimensions are *not* averaged: a single weak dimension (e.g.
        actuality=1) is a hard blocker regardless of how high the rest score.
        """
        return all(
            dim >= 3
            for dim in (
